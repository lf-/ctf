...
def make_secure():
    ...
    safe_builtins = ["repr", "str", "print", "eval", "input", "any", "exec", "all", "Exception", "getattr", "hasattr"]
...
while True:
    ...
    for char in text:
     if char not in charset:
      print('BAD', char)
    if False and (any (word in text for word in denied) or not all(char in charset for char in text)):
        print("Denied")
    else:
        try:
            exec("a= "+text)
        except Exception as error:
            print('exc', error.__class__.__name__, error.args)
